function [x4pn,e4pn,v4pn,F4pn,J4p,rho4pn,vol4pn,...
                        sigma4pn,h4pn,hbar4pn,A4pn] = I2p(...
...
parfor p = 1:nrPts
  ...
  A4pn(p,:) = -sum(NIxp'*vI(nodes,:),2).*A4p(p,:) + SA4p;
  hbar4pn(p,:) = -sum(NIxp'*vI(nodes,:),2).*hbar4p(p,:) + Sh4p;
end
h4pn = hbar4p./A4p;
rho4pn = rhop0./J4p;
vol4pn = m4p./rho4pn;
